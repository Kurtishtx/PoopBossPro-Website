...
export const metadata = {
  title: 'Storing Gate Codes and Yard Access Notes Right on the Route | PoopBossPro',
  description: 'How PoopBossPro keeps gate codes and yard access notes attached to every stop on the route so your dog-waste crews and dispatcher never get locked out.',
};
  ...
  return (
    <BlogShell>
      <article className="blog-article">
        ...
        <h1>Storing Gate Codes and Yard Access Notes Right on the Route</h1>
        <p>
        ...
        <h2>Access Notes Travel With the Stop, Not the Person</h2>
        <p>
          The old way of handling access was to keep it in one tech&apos;s head. He knew the codes, knew which gates stuck, knew where the dogs hung out &mdash; and the day he called out sick, all of that knowledge walked out the door with him. PoopBossPro attaches the gate code and access notes to the property profile, and that profile rides along with every stop on the dispatched route. When you build the day&apos;s route, each address shows its own code, its own gate location, and its own warnings. Whoever is driving that route sees exactly what the regular tech would see, because the information lives in the system instead of in a single crew member&apos;s memory.
        </p>

        <h2>What Belongs in a Yard Access Note</h2>
        <p>
        ...
        <h2>Why This Matters for the Dispatcher</h2>
        <p>
          When a tech calls out at 6 a.m., your dispatcher has to reassign that route fast. The difference between a smooth reassignment and a chaotic one is whether the access details move automatically. With PoopBossPro, the dispatcher drags those stops onto another crew member and every gate code, every yard note, every dog count comes right along with them. The new tech inherits the same information instantly &mdash; no frantic group text asking who knows the code for the blue house on Maple. This is one of the clearest reasons crews abandon paper, and our breakdown on <a href="/blogs/manual-route-sheets-vs-dispatch-software-scooping">Paper Route Sheets vs. Dispatch Software for Scooping Businesses</a> walks through just how much time a software-driven route saves when access details are never the holdup. Connecting routing, dispatch, and access notes in one place is exactly what good <a href="/pet-waste-routes-dispatch-software">routes &amp; crew dispatch software</a> is built to do.
        </p>

        <h2>Fewer Locked-Out Stops, Fewer Refunds</h2>
        <p>
